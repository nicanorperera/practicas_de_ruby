...
3 ­ Un número palíndromo se lee igual al derecho y al revés. El número palíndromo más grande obtenido de la multiplicación de dos números de 2 dígitos es 9009 (91 x 99 = 9009). Encontrá el palíndromo más grande obtenido a través de la multiplicación de dos números de 3 dígitos.
...
'003','Historias de chillar',75'003','Pegamento de personas',80
12.a ­ Crear una clase ProductsList que lo parsee y almacene la información en una colección
de productos.
12.b ­ Teniendo en cuenta la lista de productos anterior, crear una clase Purchase que procese
órdenes de compra. Dicha clase deberá:
● Implementar un método #add(producto) que agregue un producto a la orden. Si el
producto no está en la lista, no lo agregará a la compra.
● Implementar un método #total que calcule el total de los elementos agregados.
12.c ­ Incorporar lógica de procesamiento de descuentos al sistema anterior de manera que si
el total supera los $200, se aplique un descuento del 10%
12.d ­ Extender la funcionalidad para que le procesamiento de descuentos permita aplicar los
...
12.e ­ Extender las solución anterior de manera que se puedan agregar y modificar reglas
descuentos y promociones
13 ­ En un juego de combate marcial participan 2 jugadores. Cada jugador posee una energía
vital, la cual varía durante el combate. El combate se organiza en una serie de turnos, en cada
turno el jugador decide qué movimiento realizar (golpear o bloquear). El combate dura como
máximo 10 turnos, y cada jugador comienza el mismo con 100 puntos de energía. Al final del
combate puede suceder que:
...
● Ambos jugadores terminan el combate con energía mayor que 0. En este caso gana
aquel con más energía. En cada turno los jugadores pueden bloquear o golpear. Por lo
tanto de se pueden dar 3 casos:
○ El jugador A golpea y el jugador B bloquea: en este caso el que golpea (A) pierde
10 puntos de energía.
○ Ambos jugadores bloquean: en este caso la energía de ambos se mantiene igual.
○ Ambos golpean: el jugador A resta de su energía el 20% de la energía que tiene
B, y el jugador B resta el 20% de la energía que tiene A.
A modo de ejemplo considere la siguiente secuencia de turnos:
Turno Mov Jug A Mov Jug A Energía A Energía B0 ­ ­ 100 100
1 Bloqueo Bloqueo 100 100
2 Bloqueo Golpe 100 90
3 Golpe Golpe 82 70
Usted debe implementar un sistema (clase Juego) donde, dados 2 jugadores que pueden
responder con un movimiento para cada turno, determine quién gana o si hubo empate. Su
implementación debe respetar las siguiente indicaciones:
● La clase Jugador debe entender el mensaje "#jugada(nro_turno)" (donde nro_turno es un
entero entre 1 y 10) que retorna la jugada para ese turno.
● La clase Jugador debe entender el mensaje "#nombre" que retorna el nombre del
mismo.
● La clase Juego debe implementar el mensaje: "#determinar_ganador(jugador_a,
jugador_b)"
13.a ­ Implemente.
13.b ­ Instancie los jugadores con sus correspondientes movimientos y verique quien gana.